...
const NOISE_LINE_PATTERNS = [
  ...
  //
  // 추가로 확인(조경기능사 2014-10-11): 이 파일은 같은 헤더의 ◐와 ◑가 pymupdf
  // 상에서 애초에 서로 다른 line 오브젝트라, 컬럼 재정렬을 거쳐도 한 줄로 합쳐지지
  // 않고 별도 줄로 남는다(예: "◑ ... www.comcbt.com" 한 줄, "◐" 다음 줄) — 이러면
  // 위 ◐.*◑ 패턴이 둘 다 있는 한 줄을 찾지 못해 걸러내지 못한다. ◐, ◑ 둘 다 이
  // 반복 헤더에만 쓰이고(전체 파일 grep으로 확인, 실제 보기/지문에 등장한 사례
  // 0건) 함께 나올 때만 걸러낼 이유가 없으므로 단독으로도 매칭하도록 완화했다.
  // (다만 이 파일은 헤더 노이즈가 더 잘게 쪼개져 있어 이 완화만으로는 부족하고
  // 여전히 파싱에 실패한다 — 아래 참고.)
  /◐|◑/,
]
